...
#Adding the columns in the evaluator
...
def choropleth_map(request):
    ...
    #defining the variables we will use in the map
    ...
    #Giving px the values it needs to build the map
    fig = px.choropleth(
        geojson_data,
    ...
    #Defining the boundaries of map elements and visibility of borders
    fig.update_geos(fitbounds="locations", visible=False)
    # Set the initial zoom level and map size 
    fig.update_layout(
        autosize=False,
        margin = dict(
                l=0,
    ...
    #Creating the graph element built to serve in html
...
def home(request):
    ...
    if request.method == 'POST':
        ...
        if form.is_valid():
            ...
            # Load the saved model and scaler (need to change path if running elsewhere!)
